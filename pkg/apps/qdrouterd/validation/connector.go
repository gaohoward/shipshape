...
func ValidateDefaultConnectors(interconnect *v1alpha1.Interconnect, c framework.ContextData, pods []v1.Pod) {
	...
	for _, pod := range pods {
		// Retrieve connectors
		connectors, err := qdrmanagement.QdmanageQuery(c, pod.Name, entities.Connector{}, nil)
		gomega.Expect(err).To(gomega.BeNil())

		// Common connector properties
		expectedPort := "55672"
		expectedSslProfile := ""
		if c.CertManagerPresent {
			expectedPort = "55671"
...
func ValidateSpecConnector(ic *v1alpha1.Interconnect, c framework.ContextData, cMap ConnectorMapByPort) {
	...
	for _, pod := range icNew.Status.PodNames {
		...
		// Retrieve connectors
		connectors, err := qdrmanagement.QdmanageQuery(c, pod, entities.Connector{}, nil)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
